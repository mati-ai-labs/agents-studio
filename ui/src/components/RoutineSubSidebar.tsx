...
export function RoutineSubSidebar({
  ...
  const visibleGroups = hideTriggers
    ? NAV_GROUPS.map((group) => ({ ...group, items: group.items.filter((item) => item.key !== "triggers") })).filter((group) => group.items.length > 0)
    : NAV_GROUPS;
  ...
  const focusItem = (index: number) => {
    ...
    onNavigate(visibleItems[clamped].key);
  };
