...
export const routineRuns = pgTable(
  "routine_runs",
  {
    ...
    callbackIssueId: uuid("callback_issue_id").references(() => issues.id, { onDelete: "set null" }),
  ...
  (table) => ({
    ...
    callbackIssueIdx: index("routine_runs_callback_issue_idx").on(table.callbackIssueId),
