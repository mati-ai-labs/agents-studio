...
export async function createApp(
  ...
  api.use(
    pluginRoutes(
      ...
      { scheduler, jobStore },
      { workerManager },
      { toolDispatcher: unifiedDispatcher },
      { workerManager },
      { toolGateway },
    ),
  );
  ...
  void unifiedDispatcher.initialize().catch((err) => {
    logger.error({ err }, "Failed to initialize plugin and connector tool dispatchers");
  });
