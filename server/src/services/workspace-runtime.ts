...
async function attachPaperclipPreviewLease(input: {
  ...
  const expose = parseObject(input.service.expose);
  if (
    asString(expose.type, "") !== "paperclip_preview"
    && input.record.stopPolicy?.paperclipPreview !== true
  ) {
    return input.record;
  }
  ...
  for (const service of rawServices) {
    ...
    if (reuseKey) {
      ...
      if (existing && existing.status === "running") {
        ...
        await attachPaperclipPreviewLease({ db: persistenceDb, record: existing, service });
        refs.push(toRuntimeServiceRef(existing, { reused: true }));
        continue;
      }
    }
    ...
    await attachPaperclipPreviewLease({ db: persistenceDb, record: started.record, service });
  ...
  try {
    ...
    for (const pending of startBatch.pendingReadiness) {
      try {
        ...
        await attachPaperclipPreviewLease({ db: input.db, record: pending.record, service: {} });
