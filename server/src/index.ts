...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    const startupCompletionDeliveries = await issueCompletionDeliveries.processDueDeliveries();
    if (startupCompletionDeliveries.claimed > 0) {
      logger.info(
        { ...startupCompletionDeliveries },
        "startup issue completion delivery sweep processed queued Slack deliveries",
      );
    }
    ...
    heartbeatSchedulerInterval = setInterval(() => {
      ...
      void (async () => {
        ...
        if (heartbeatSchedulerStopped) return;
        trackHeartbeatSchedulerWork(issueCompletionDeliveries
          .processDueDeliveries()
          .then((deliveries) => {
            if (deliveries.claimed > 0) {
              logger.info(
                { ...deliveries },
                "periodic issue completion delivery sweep processed queued Slack deliveries",
              );
            }
          })
          .catch((err) => {
